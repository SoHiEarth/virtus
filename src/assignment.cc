...
void SaveAssignmentsToDatabase(const std::vector<Assignment>& assignments) {
  ...
  for (const auto& assignment : assignments) {
    ...
    assignment_node.append_child("class_name").text() =
        assignment.class_name.data();
    assignment_node.append_child("description").text() =
        assignment.description.data();
    assignment_node.append_child("due_date").text() =
        assignment.due_date.data();
        ...
        case 5: {  // Score
          while (true) {
            ...
            try {
              ...
              mvprintw(LINES - 1, 0,
                       "Invalid score. Please enter a valid value.\n");
              refresh();
              getch();
            }
          }
        ...
        case 6: {  // Max Score
          while (true) {
            ...
            try {
              ...
              mvprintw(LINES - 1, 0,
                       "Invalid max score. Please enter a valid value.\n");
              refresh();
              getch();
            }
          }
...
void AssignmentMenu(Assignment* assignment) {
  if (assignment) AssignmentMenu(*assignment);
}
